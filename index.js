 ...
 */
export function setTimedTask(customTimerName, timerCallback, executionTime) {
    // 执行函数
    timerCallback();
    // 将定时器信息保存到Map中,方便管理
 ...
 * @param { number } executionTime 定时器的执行时间
 */
export function restartTimedTask(customTimerName, executionTime) {
    // 判断Map中是否存在key值
    if (timerTaskMap.has(customTimerName)) {
        // 获取 [ <定时器ID> , <定时器执行回调函数> ]
        const [timerID, timerCallback] = timerTaskMap.get(customTimerName);
        // 清除定时器
 ...
 * @param { string } customTimerName 已被管理的定时器名称
 */
export function clearTimedTask(customTimerName) {
    // 判断Map中是否存在key值
    if (timerTaskMap.has(customTimerName)) {
        // 获取 [ <定时器ID> , <定时器执行回调函数> ]
        const [timerID] = timerTaskMap.get(customTimerName);
        // 清除定时器
        clearInterval(timerID);
        // 删除Map的项
